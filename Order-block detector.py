...
client = Client(api_key, api_secret)  # Ensure client is initialized here
...
def calculate_beta_metrics(data1, data2, risk_free_rate=0.02):
    ...
    return {
        'beta': beta,
        'volatility_token1': volatility1,
        'volatility_token2': volatility2,
        'sharpe_token1': sharpe1,
        'sharpe_token2': sharpe2
    }
...
class BetaDistributionAnalysis(param.Parameterized):
    tokens = param.ListSelector(default=['ETHUSDT'], objects=['ETHUSDT', 'BNBUSDT', 'XRPUSDT', 'ADAUSDT', 'DOGEUSDT'])
    custom_token = param.String(default='')
    ...
    def __init__(self, **params):
        ...
        self.available_tokens = self.get_available_tokens()
        self.param.tokens.objects = self.available_tokens
        self.custom_tokens = set()  # Store valid custom tokens
        self.update_data()
    ...
    def validate_token(self, token):
        ...
        try:
            test_data = get_spot_historical_data(token, self.timeframe, limit=1)
            return not test_data.empty
        except Exception:
            return False

    def add_custom_token(self, token):
        """Add a custom token if it's valid"""
        token = token.strip().upper()  # Normalize token string
        if not token.endswith('USDT'):
            token = f"{token}USDT"

        if self.validate_token(token):
            self.custom_tokens.add(token)
            current_objects = list(self.param.tokens.objects)
            if token not in current_objects:
                current_objects.append(token)
                self.param.tokens.objects = current_objects
                # Automatically select the new token for display in the plot
                self.tokens = list(self.tokens) + [token]  # Append to tokens list
            return True, f"Successfully added {token}"
        return False, f"Invalid token pair: {token}"

    def calculate_rolling_beta(self, btc_data, token_data):
        """Calculate rolling beta values for a token relative to BTC"""
        btc_returns = btc_data['close'].pct_change().dropna()
        token_returns = token_data['close'].pct_change().dropna()

        # Align the data
        aligned_data = pd.concat([btc_returns, token_returns], axis=1).dropna()
        btc_returns = aligned_data.iloc[:, 0]
        token_returns = aligned_data.iloc[:, 1]

        # Calculate rolling beta
        rolling_cov = token_returns.rolling(window=self.window_size).cov(btc_returns)
        rolling_var = btc_returns.rolling(window=self.window_size).var()
        rolling_beta = rolling_cov / rolling_var

        return rolling_beta.dropna()

    @param.depends('tokens', 'timeframe', 'window_size', 'update_frequency', 'custom_token', watch=True)
    def update_data(self):
        if self.custom_token:
            success, message = self.add_custom_token(self.custom_token)
            self.custom_token = ''  # Reset input after adding
        ...
        try:
            ...
            for token in self.tokens:
                token_data = get_spot_historical_data(token, self.timeframe)
                if not token_data.empty:
                    beta_values = self.calculate_rolling_beta(btc_data, token_data)
                    token_betas[token] = beta_values
            ...
            for (token, beta_values), color in zip(token_betas.items(), colors):
                mean_beta = np.mean(beta_values)
                std_beta = np.std(beta_values)
                ...
                token_curve = hv.Curve((x, pdf), 'Beta', 'Density',
                                       label=f'{token} (β={mean_beta:.2f})').opts(
                    color=color, line_width=2)
                dist_plot = dist_plot * token_curve
            ...
            for stat in beta_stats:
                stats_md += f"| {stat['Token']} | {stat['Mean Beta']:.3f} | {stat['Std Dev']:.3f} | {stat['Min Beta']:.3f} | {stat['Max Beta']:.3f} |\n"
    ...
    def view(self):
        ...
        custom_token_input = pn.widgets.TextInput(
            name='Add Custom Token (e.g., "ETH" or "ETHUSDT")',
        ...
        def add_token_callback(event):
            if event.new and event.new.strip():
                success, message = self.add_custom_token(event.new)
                if success:
                    custom_token_input.value = ''  # Clear the input
                custom_token_input.name = message  # Show feedback in the label

        custom_token_input.param.watch(add_token_callback, 'value')

        # Create widgets
        tokens_select = pn.widgets.MultiChoice(
            name="Select Tokens for Analysis",
            value=self.tokens,
            options=self.param.tokens.objects,
            sizing_mode='stretch_width'
        )
        tokens_select.link(self, value='tokens')
...
class BetaDistributionAnalysis(param.Parameterized):
    ...
    def add_custom_token(self, token):
        ...
        if self.validate_token(token):
            self.custom_tokens.add(token)
            if token not in self.param.tokens.objects:
                self.param.tokens.objects = list(self.param.tokens.objects) + [token]
            if token not in self.tokens:
                self.tokens = self.tokens + [token]  # Add to selected tokens list to display
            return True, f"Successfully added {token}"
        return False, f"Invalid token pair: {token}"
    ...
    def update_data(self):
        ...
        if self.custom_token:
            success, message = self.add_custom_token(self.custom_token)
            self.custom_token = ''  # Clear input after attempting to add
...
class EnhancedCorrelationAnalysis(param.Parameterized):
    base_token = param.String(default='BTCUSDT')
    comparison_tokens = param.ListSelector(default=[], objects=['ETHUSDT', 'BNBUSDT', 'XRPUSDT', 'ADAUSDT', 'DOGEUSDT'])
    timeframe = param.Selector(default='1h', objects=['1m', '5m', '15m', '30m', '1h', '4h', '1d'])
    window_size = param.Integer(default=30, bounds=(5, 100))
    update_frequency = param.Integer(default=0, bounds=(0, 3600))
...
class CorrelationAnalysis(param.Parameterized):
    token1 = param.String(default='BTCUSDT')
    token2 = param.String(default='ETHUSDT')
    timeframe = param.Selector(default='1h', objects=['1m', '5m', '15m', '30m', '1h', '4h', '1d'])
    window_size = param.Integer(default=30, bounds=(5, 100))
    update_frequency = param.Integer(default=0, bounds=(0, 3600))
    ...
    def __init__(self, **params):
        self.client = Client(api_key=api_key, api_secret=api_secret)  # Initialize client here
    ...
    def calculate_hedge_metrics(self, data1, data2):
        ...
        try:
            ...
            funding_rate1 = self.get_funding_rate(self.token1)
            funding_rate2 = self.get_funding_rate(self.token2)
    ...
    def update_data(self):
        try:
            data1 = get_spot_historical_data(self.token1, self.timeframe)
            data2 = get_spot_historical_data(self.token2, self.timeframe)

            if data1.empty or data2.empty:
                self.correlation_info.object = "Error: Unable to fetch data for one or both tokens"
                self.plot_pane.object = None
                return
            ...
            normalized_df = pd.DataFrame({
                'timestamp': df.index,
                self.token1: df['price1'] / df['price1'].iloc[0] * 100,
                self.token2: df['price2'] / df['price2'].iloc[0] * 100
            }).reset_index(drop=True)

            price_plot = (
                    hv.Curve(normalized_df, 'timestamp', self.token1, label=self.token1).opts(
                        color='blue', line_width=2
                    ) *
                    hv.Curve(normalized_df, 'timestamp', self.token2, label=self.token2).opts(
            ...
            if self.show_risk_analysis:
                try:
                    data1.name = self.token1
                    data2.name = self.token2
            ...
            if self.show_beta:
                ...
                beta_info = f"""
                        ...
                        • {self.token1} Metrics:
                          - Volatility: {beta_metrics['volatility_token1']:.4f}
                          - Sharpe Ratio: {beta_metrics['sharpe_token1']:.4f}

                        • {self.token2} Metrics:
                          - Volatility: {beta_metrics['volatility_token2']:.4f}
                          - Sharpe Ratio: {beta_metrics['sharpe_token2']:.4f}
                        """
                analysis_results += f"\n{beta_info}\n"
            ...
            if self.show_beta:
                ...
                ### Beta Analysis
                - Beta: {beta_metrics['beta']:.4f}
                - {self.token1} Volatility: {beta_metrics['volatility_token1']:.4f}
                - {self.token2} Volatility: {beta_metrics['volatility_token2']:.4f}
                - {self.token1} Sharpe Ratio: {beta_metrics['sharpe_token1']:.4f}
                - {self.token2} Sharpe Ratio: {beta_metrics['sharpe_token2']:.4f}
                """
            ...
            if self.show_granger:
                ...
                granger_info = f"""
                ### Granger Causality
                - {self.token1} → {self.token2} p-value: {p_value_1_2:.4f}
                - {self.token2} → {self.token1} p-value: {p_value_2_1:.4f}
                """
            ...
            if self.show_dtw:
                ...
                if dtw_results['success']:
                    ...
                    path_plot = hv.Curve(warping_df, 'index1', 'index2').opts(
                        ...
                        color='purple',
                        line_width=1,
                        xlabel=f'{self.token1} Index',
                        ylabel=f'{self.token2} Index',
                        show_grid=True
                    )
                ...
                try:
                    ...
                    if ef_results[0] is not None:
                        ...
                        **Minimum Volatility Portfolio:**
                        ...
                        - Weights: {self.token1}: {min_vol_port[self.token1]:.2%}, {self.token2}: {min_vol_port[self.token2]:.2%}
                        ...
                        **Maximum Sharpe Ratio Portfolio:**
                        ...
                        - Weights: {self.token1}: {max_sharpe_port[self.token1]:.2%}, {self.token2}: {max_sharpe_port[self.token2]:.2%}
                        """
                    else:
                        risk_info = "\n### Portfolio Risk Analysis\nUnable to calculate risk metrics."
    ...
    def view(self):
        ...
        token1_input = pn.widgets.TextInput(
            name="Token 1",
            value=self.token1,
            sizing_mode='stretch_width'
        )
        token1_input.link(self, value='token1')

        token2_input = pn.widgets.TextInput(
            name="Token 2",
            value=self.token2,
            sizing_mode='stretch_width'
        )
        token2_input.link(self, value='token2')
